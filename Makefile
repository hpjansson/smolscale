...
test-simd: Makefile smolscale.c smolscale.h png.c test.c $(SKIA_OBJ)
	gcc -Wall -Wextra -fverbose-asm -Ofast -mfma -flto -fstrict-aliasing -ftree-vectorize -mcpu=native -march=native -mtune=native -mmmx -msse4 -msse4.2 -msse4.1 -msse2 -msse3 -msse -mavx -mavx2 -g $(TEST_DEPS) $(SKIA_CFLAGS_$(WITH_SKIA)) $(SKIA_LDFLAGS_$(WITH_SKIA)) smolscale.c png.c test.c -o test-simd
